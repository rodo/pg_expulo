...
func generateConfig(filename string, tables []dbTable) {
	...
	var defaults []defColumn
	log.Debug(defaultsFile)
	if len(defaultsFile) == 0 {
		...
		defaults = readDefaultsFile(defaultsFile)
	}
...
// Read defaults definition file in json from disk
func readDefaultsFile(filename string) []defColumn {
	...
	// we unmarshal our byteArray which contains our
	// jsonFile's content into our main Struct Config
	if err := json.Unmarshal(byteValue, &defaults); err != nil {
		panic(err)
	}
	jsonFile.Close()
	return defaults
